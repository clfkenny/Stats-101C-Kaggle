...
#p3 <- predict(svm_mod, testing_X, type = 'probability')
p_df <- data.frame(p1,p2)
...
svm_mod <- svm(Fatal ~ ., data = train_data_train, cost=5, gamma = 0.01, probability = T)
svm_mod_pred_test <- predict(svm_mod, newdata = train_data_test, type='probability')
...
xgb <- xgboost(data = train_X, label = train_data_train$response_dummy, params = xgb_params_1, nrounds=46, subsample = 1, gamma = 0)
...
test_df <- data.frame('id' = fin_test$id,
                      'Fatal' ='No')
test_df$Fatal <- as.character(test_df$Fatal)
test_df[xgb_yes_rows, 'Fatal'] <- 'Yes'
...
xgb_pred_test <- predict(xgb, final_test_X)[,-1]
...
write.csv(test_df, '~/dropbox/stats-101c-kaggle/Submissions/Submission29.csv', row.names=F)
