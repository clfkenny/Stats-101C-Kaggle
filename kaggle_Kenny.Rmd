...
fatal_index <- unique(c(i,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12,i13))
fatal_ids <- final_test[fatal_index, 'id']

non_i1 <- which(!is.na(str_locate(final_test$FullNarrative, 'Non-Fatal')[,1]) == T)
non_fatal_ids <- final_test[c(556,63,1241,1193,1225, non_i1),'id']
...
# 
# View(data[which(!is.na(str_locate(data$FullNarrative, 'treat')[,1]) == T),])
...
write.csv(training, '~/dropbox/stats-101c-kaggle/Datasets/cleaned_merged1.csv', row.names=F)
...
#training[training$AgeGroup==0, 'AgeGroup'] <- 5
#training$AgeGroup <- as.factor(training$AgeGroup)
training$races <- as.factor(paste(training$SubjectRace, training$mainOfficerRace))
training$state <- as.factor(training$state)
training$City <- as.factor(training$City)
training[training$SubjectGender=='U', 'SubjectGender'] <- 'M'

train_data <- na.omit(training[, c('Fatal', 'SubjectGender', 'SubjectArmed',
                                   'NewAge', 'NumberOfOfficers', 'ShotsClean', 'ShotsClean',
                                   'Year', 'Month', 'Day','estimate2016', 'SubjectRace')
                                  ]) 
...
n <- nrow(train_data)
set.seed(seed)
train_i <- sample(1:n,n*0.8,replace=F )
...
rf_grid_1 <- expand.grid(.mtry=c(2:20))
set.seed(seed)
...
set.seed(seed)
rF_mod <- randomForest(Fatal ~., data = train_data_train,ntree=200, 
                       mtry=4, importance=T, maxnode=150)
rF_mod
...
svm_trcontrol_1 <- trainControl(method="cv", number=5, verboseIter = T, returnResamp = 'all',
                              allowParallel = T, savePredictions = T, classProbs = T)
svm_grid_1 <- expand.grid(cost=c(0.01, 0.1,1,10))
set.seed(seed)
...
svm_mod <- svm(Fatal ~ ., data = train_data_train, cost=100, gamma = 0.01)
svm_mod_pred_test <- predict(svm_mod, newdata = train_data_test)
...
xgb <- xgboost(data = train_X, label = train_data_train$response_dummy, params = xgb_params_1, nrounds=21)
xgb_pred <- predict(xgb, test_X)
xgb_pred_class <- ifelse(xgb_pred >= 0.5, 'F', 'N')
...
write.csv(test_df, '~/dropbox/stats-101c-kaggle/Submissions/Submission17.csv', row.names=F)
