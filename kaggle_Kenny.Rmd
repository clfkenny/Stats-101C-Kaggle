...
fatal_index <- unique(c(i,i2,i3,i4,i5,i6,i7,i8,i9,i10,i11,i12))
...
# View(data[which(!is.na(str_locate(data$Notes, 'justif')[,1]) == T),])
...
# train_data <- na.omit(training[, c('Fatal', 'SubjectGender', 'SubjectRace',
#                                        'ShotsClean',
#                                    'NewAge', 'NumberOfOfficers'
#                                    ,'Month', 'Day','estimate2016', 'mainOfficerRace', 'income'
#                                    )]) 

  

write.csv(merged, '~/dropbox/stats-101c-kaggle/Datasets/clean_merged3.csv', row.names=F)
...
testing_F <- testing[testing$Fatal == 'F',][sample(1:191, 70),]
testing_NF <- testing[testing$Fatal != 'F', -48]

training_w_testing <- rbind(training, testing_F)


train_data <- na.omit(training[,c('Fatal','SubjectRace', 'SubjectGender', 'SubjectArmed',
                                   'scaled_log_new_age', 'NumberOfOfficers', 'ShotsClean',
                                   'Year','Seasons','estimate2016' ,'scaled_log_income',
                                   'city_fatal_prop')])


#prop.table(table(predict(rF_mod, newdata=testing)))
...
n <- nrow(train_data)
set.seed(seed)
train_i <- sample(1:n,n, replace=F )
...
set.seed(seed)
rF_mod <- randomForest(Fatal ~., data = train_data_train,ntree=1000, 
...
xgb <- xgboost(data = train_X, label = train_data_train$response_dummy, params = xgb_params_1, nrounds=46, subsample = 0.9, gamma = 0)
...
write.csv(test_df, '~/dropbox/stats-101c-kaggle/Submissions/Submission23.csv', row.names=F)
